...
class ClientCacheValue {
    public:
    ClientCacheValue(){};
    ClientCacheValue(const CacheFileInfo &file_Attrs,  bool isDirty, int fileDiscriptor) : fileInfo(file_Attrs), isDirty(false), fileDiscriptor(fileDiscriptor)  {};
    ...
    void saveCache() {
        ...
        if (cache_file) {
            for (auto it = cache.begin(); it != cache.end(); it++) {
                ...
                cache_file << key << ":" << ":" << (value.isDirty ? 1 : 0) << ":" << value.fileDiscriptor <<":"<< value.fileInfo.st_dev << ":" << value.fileInfo.st_ino<<":"<<value.fileInfo.st_mode<<":"<<value.fileInfo.st_nlink<<":"<<value.fileInfo.st_uid<<":"<<value.fileInfo.st_gid<<":"<<value.fileInfo.st_rdev<<":"<<value.fileInfo.st_size<<":"<<value.fileInfo.st_blksize<<":" << value.fileInfo.st_blocks<<":"<<value.fileInfo.st_atim << ":" << value.fileInfo.st_mtim << ":" <<value.fileInfo.st_ctim << ":" <<std::endl;
            }
            cache_file.close();
        }
