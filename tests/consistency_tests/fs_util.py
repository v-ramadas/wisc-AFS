...
def start_winner_test(host: str, test_case: int, client_id: str,
                         signal_fname: str):
    ...
    script_name = f'~/scripts/test_winner.py'
    ssh_cmd = f'source ~/scripts/739p1.env && python {script_name}'
...
def start_another_client_opname(host: str, test_case: int, client_id: str,
                         signal_fname: str, opname: str):
    """Invoke the python script for another client via ssh.
        REQUIRED: $USERNAME/.ssh/id_rsa.pub is added to remote client's
        ~/.ssh/authorized_keys , or ensure client.connect() can succeed.
        REQUIRED: put 739p1.env into ~/
    ...
    Args:
        ...
        signal_fname (str): started remote client will wait for this signal.
            ...
            so this client can know it.
    """
    print("inside opname with",opname)
    send_signal(host, signal_fname)
    signal_exists = (not poll_signal_remove(host, signal_fname))
    assert signal_exists
    client = paramiko.SSHClient()
    ...
    username = getpass.getuser()
    home_dir = os.path.expanduser('~')
    script_name = f'{home_dir}/scripts/test{test_case}_client{client_id.upper()}_{opname}.py'
    ssh_cmd = f'source {home_dir}/scripts/739p1.env && python {script_name}'
    print(ssh_cmd)
    print(username)
    print(home_dir)
    assert username is not None
...
def send_signal(host: str, signal_fname: str):
    ...
    outs, _errs, ret = get_shell_cmd_output(None, ssh_cmd)
    ssh_cmd = f'ssh {host} chmod 0777 {signal_fname}'
    outs, _errs, ret = get_shell_cmd_output(None, ssh_cmd)
    for lo in outs:
        print(lo)
    assert ret == 0
